...
_self = {
    ...
    shouldBypassProxy: function (url) {
        var host = utils.parseUrl(url).host;
        var bypassList = constants.PROXY.BYPASS_LIST;

        for (var i = 0, len=bypassList.length; i < len; i++) {
            var bypassUrl = bypassList[i];
            if (host.indexOf(bypassUrl) != -1) {
                return true;
            }
        }       
